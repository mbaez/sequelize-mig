...
export const getActions = async (oldTables, newTables, upActions = null) => {
  ...
  /*
   * We clone the objects here before the diff to avoid issues with deep diff.
   * The original objects has additional methods that are not related with the model schema and
   * that are note present in the new schema (Eg. 'toString'.)
   */
  const oldTablesClone = JSON.parse(JSON.stringify(oldTables))
  const newTablesClone  = JSON.parse(JSON.stringify(newTables))

  const initialDifferences = deepDiff.diff(oldTablesClone, newTablesClone);
